...
// --- Supabase Initialization ---
    ...
    const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, supabaseOptions);
    ...
    function renderActiveUsers(state) {
      ...
      filteredUsers.sort((a, b) => {
        ...
        usersList.innerHTML = filteredUsers.map(u => {
          ...
          // Match role config case-insensitively
          const roleKey = Object.keys(ROLE_CONFIG).find(k => k.toLowerCase() === rawRole.toLowerCase()) || 'Default';
          const config = ROLE_CONFIG[roleKey];
          
          const roleDisplayName = roleKey === 'Default' ? rawRole : roleKey;
          const roleClass = roleDisplayName.toLowerCase().replace(/\s+/g, '-');
